"""
Script 17: Extract Recovery Guide Template
...
def create_generic_template():
    """Create generic Recovery Guide template"""
    template = """# Database Recovery Guide
...
def extract_and_archive():
    ...
    old_file = repo_root / "docs" / "deployment" / "RECOVERY_GUIDE.md-old"
    template_file = repo_root / "docs" / "deployment" / "RECOVERY_GUIDE.md"
    archive_dir = repo_root / "docs" / "archive" / "deployments"
    archive_file = archive_dir / "RECOVERY_GUIDE_MAGERSTAV_2025-11-21.md"
    archive_index = repo_root / "docs" / "archive" / "00_ARCHIVE_INDEX.md"
    doc_index = repo_root / "docs" / "00_DOCUMENTATION_INDEX.md"

    print("🔄 Extracting Recovery Guide Template...")
    print()
    ...
    print("   Generic Recovery Guide template")
    print()
    ...
    if archive_index.exists():
        ...
        if "## Deployment Records" in content:
            lines = content.split('\n')
            # Find operations guide entry and add after it
            insert_idx = None
            for i, line in enumerate(lines):
                if "Operations Guide" in line and "2025-11-24" in line:
                    insert_idx = i + 1
                    break

            if insert_idx:
                new_entry = "- **2025-11-21** - [Mágerstav Recovery Guide](deployments/RECOVERY_GUIDE_MAGERSTAV_2025-11-21.md) - Database disaster recovery procedures"
        ...
        print("   Added recovery guide entry")
    ...
    if doc_index.exists():
        ...
        if deployment_section in content:
            ...
            # Find OPERATIONS_GUIDE entry and add after it
            for i, line in enumerate(lines):
                if "Operations Guide" in line:
                    insert_idx = i + 1
                    break

            if insert_idx:
                new_entry = "- [Recovery Guide](deployment/RECOVERY_GUIDE.md) - Database disaster recovery procedures"
                lines.insert(insert_idx, new_entry)
                content = '\n'.join(lines)
        ...
        print("   Added RECOVERY_GUIDE.md to deployment section")
    ...
    print()
    print("Summary:")
    print(f"  • Created: docs/deployment/RECOVERY_GUIDE.md (template)")
    print(f"  • Archived: RECOVERY_GUIDE_MAGERSTAV_2025-11-21.md")

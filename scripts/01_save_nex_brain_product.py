#!/usr/bin/env python
"""
Save NEX Brain Product Strategy document.
Creates: docs/strategic/NEX_BRAIN_PRODUCT.md
Updates: docs/strategic/00_STRATEGIC_INDEX.md
"""

from pathlib import Path

# Document content
NEX_BRAIN_CONTENT = '''# NEX Brain - Product Strategy Document
...
def main():
    ...
    strategic_path = base_path / "docs" / "strategic"

    # 1. Save NEX_BRAIN_PRODUCT.md
    doc_path = strategic_path / "NEX_BRAIN_PRODUCT.md"
    doc_path.write_text(NEX_BRAIN_CONTENT, encoding="utf-8")
    print(f"✅ Created: {doc_path}")

    # 2. Update 00_STRATEGIC_INDEX.md
    index_path = strategic_path / "00_STRATEGIC_INDEX.md"
    if index_path.exists():
        content = index_path.read_text(encoding="utf-8")

        # Add NEX Brain to index if not present
        if "NEX_BRAIN_PRODUCT.md" not in content:
            # Find insertion point after RAG_IMPLEMENTATION
            insert_marker = "### Ready for Implementation"
            new_entry = '''### Ready for Implementation

**[NEX_BRAIN_PRODUCT.md](NEX_BRAIN_PRODUCT.md)**
- NEX Brain - Inteligentné rozhranie pre NEX ekosystém
- RAG + Ollama architektúra, use cases, implementačné fázy
- Pilot plán: ICC s.r.o., ANDROS s.r.o.
- Status: 📋 Planning
- Veľkosť: ~25 KB
...
'''
            if insert_marker in content:
                ...
                print(f"⚠️ Could not find insertion point in {index_path}")
    else:
        print(f"⚠️ Index not found: {index_path}")

    print("\n✅ NEX Brain Product Strategy document saved!")
    print("\nNext steps:")
    print("1. Review document in docs/strategic/NEX_BRAIN_PRODUCT.md")
    print("2. Run: python tools/rag/rag_update.py --new")

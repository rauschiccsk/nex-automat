...
| SupPac | longint | supplier_id | INTEGER | Primárny dodávateľ produktu | ✓ |
...
- last_supplier_id = posledný dodávateľ (z príjemky), supplier_id v product_catalog = primárny/preferovaný dodávateľ
...
- SupPac (dodávateľ) sa mapuje priamo v `product_catalog.supplier_id`
- PrdPac (výrobca) sa mapuje cez samostatnú tabuľku `product_catalog_partners` → `partner_catalog` (TODO)
...
| Sended | byte | Zastaraný (sync flag) |
| ModNum | word | PostgreSQL má verziu cez trigger |
| IsiSnt | Str3 | Účtovné prerozúčtovanie - zastaraný |
| IsiAnl | Str6 | Účtovné prerozúčtovanie - zastaraný |
| IciSnt | Str3 | Účtovné prerozúčtovanie - zastaraný |
| IciAnl | Str6 | Účtovné prerozúčtovanie - zastaraný |
...
- **supplier_id** = Primárny/preferovaný dodávateľ produktu (SupPac)
- **last_supplier_id** (v extensions) = Posledný dodávateľ z príjemky (LinPac)
...
- Skontrolovať foreign keys (package_product_id, vat_group_id, supplier_id)
...
**Aktualizované:** 2025-12-28  
**Verzia:** 1.3  
**Zmeny v 1.3:** Pridané pole supplier_id (SupPac) pre primárneho dodávateľa  
**Status:** ✅ Pripravené na migráciu
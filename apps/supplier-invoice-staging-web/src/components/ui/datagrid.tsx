...
export const numericFilter = (row: any, columnId: string, filterValue: string): boolean => {
  ...
  if (!isNaN(exactValue)) return numValue === exactValue;
  return false;
};
...
function ColumnWidthInput({ value, onChange }: { value: number; onChange: (value: number) => void; }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <input type="text" inputMode="numeric" pattern="[0-9]*" value={localValue} onChange={(e) => setLocalValue(e.target.value.replace(/[^0-9]/g, ''))}
        onBlur={handleBlur} onKeyDown={handleKeyDown} className="w-14 px-2 py-1 h-7 text-xs text-center border rounded border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500" />
...
function EditableCell({ value, rowId, columnId, onSave, type = 'number', formatDisplay }: EditableCellProps) {
  ...
  if (isEditing) {
    return (
      <input
        ...
        className="w-full px-1 py-0 text-xs border border-blue-500 rounded bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 text-right"
      />
    );
  }
  ...
  return (
    <div
      ...
      className="text-right cursor-pointer hover:bg-blue-50 px-1 py-0.5 rounded -mx-1 -my-0.5"
...
export function DataGrid<T extends { id: number | string }>({
  ...
  const handleColumnOrderChange = useCallback((newOrder: ColumnOrderState) => {
    setColumnConfig(prev => prev.map(col => ({ ...col, order: newOrder.indexOf(col.id) })).sort((a, b) => a.order - b.order));
  }, []);
  ...
  const clearAllFilters = () => { setColumnFilters([]); filterInputRefs.current[0]?.focus(); };
    ...
    <div className={cn('flex flex-col border rounded-lg bg-white overflow-hidden max-w-full', className)}>
      <div className="px-3 py-1 bg-slate-50 border-b flex items-center justify-between text-xs">
        <span className="text-slate-600">
          ...
          {activeFiltersCount > 0 && <span className="ml-2 text-blue-600">• {activeFiltersCount} {activeFiltersCount === 1 ? 'filter' : 'filtre'} aktívne</span>}
        </span>
        <div className="flex items-center gap-2">
          {activeFiltersCount > 0 && <button onClick={clearAllFilters} className="text-slate-500 hover:text-slate-700 flex items-center gap-1"><X className="h-3 w-3" /> Zrušiť filtre</button>}
          <Dialog open={configOpen} onOpenChange={setConfigOpen}>
            <DialogTrigger asChild><button className="p-1.5 rounded hover:bg-slate-200 text-slate-600 hover:text-slate-900" title="Nastavenie stĺpcov"><Settings2 className="h-4 w-4" /></button></DialogTrigger>
            <DialogContent className="max-w-xl">
              <DialogHeader><DialogTitle>Nastavenie stĺpcov</DialogTitle></DialogHeader>
              <p className="text-xs text-slate-500 -mt-2 mb-2">Potiahnutím ⋮⋮ zmeníte poradie stĺpcov</p>
              <div className="space-y-1 max-h-96 overflow-y-auto">
                {sortedConfig.map((col) => (
                  <div key={col.id} onDragOver={(e) => handleDialogDragOver(e, col.id)} onDragLeave={handleDialogDragLeave} onDrop={(e) => handleDialogDrop(e, col.id)}
                    className={cn("flex items-center gap-2 p-2 rounded border transition-all", col.visible ? "bg-white" : "bg-slate-50 opacity-60",
                      dialogDraggedId === col.id && "opacity-50 border-dashed border-slate-400", dialogDragOverId === col.id && "border-blue-500 border-2 bg-blue-50")}>
                    <div draggable onDragStart={(e) => handleDialogDragStart(e, col.id)} onDragEnd={handleDialogDragEnd} className="cursor-grab active:cursor-grabbing p-1 text-slate-400 hover:text-slate-600" title="Potiahnite pre zmenu poradia"><GripVertical className="h-4 w-4" /></div>
                    <button type="button" onClick={() => handleVisibilityToggle(col.id)} className={cn("p-1 rounded hover:bg-slate-100", col.visible ? "text-green-600" : "text-slate-400")} title={col.visible ? "Skryť stĺpec" : "Zobraziť stĺpec"}>{col.visible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}</button>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1"><code className="text-xs px-1.5 py-0.5 bg-slate-100 rounded text-slate-600 font-mono truncate">{col.id}</code></div>
                      <ColumnHeaderInput value={col.customHeader ?? col.originalHeader} placeholder={col.originalHeader} onChange={(value) => handleHeaderChange(col.id, value)} />
                    </div>
                    <div className="flex flex-col items-end gap-1"><span className="text-xs text-slate-400">Šírka</span><ColumnWidthInput value={getColumnWidth(col.id)} onChange={(width) => handleWidthChange(col.id, width)} /></div>
                  </div>
                ))}
              </div>
              <div className="flex justify-between pt-4 border-t">
                <Button type="button" variant="ghost" onClick={handleResetConfig}><RotateCcw className="h-4 w-4 mr-2" /> Obnoviť pôvodné</Button>
                <div className="flex gap-2"><Button type="button" variant="secondary" onClick={() => setConfigOpen(false)}>Zrušiť</Button><Button type="button" onClick={handleSaveConfig}>Uložiť</Button></div>
              </div>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <div ref={tableContainerRef} className="overflow-auto flex-1 min-h-0">
        <table className="w-full border-collapse" style={{ width: table.getTotalSize() }}>
          <thead className="sticky top-0 z-10">
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className="bg-slate-100">
                {headerGroup.headers.map((header) => (
                  <th key={header.id} draggable={!isResizing} onDragStart={(e) => { if (!isResizing) handleHeaderDragStart(e, header.column.id); }} onDragEnd={handleHeaderDragEnd}
                    onDragOver={(e) => { if (!isResizing) handleHeaderDragOver(e, header.column.id); }} onDragLeave={handleHeaderDragLeave} onDrop={(e) => { if (!isResizing) handleHeaderDrop(e, header.column.id); }}
                    className={cn("px-2 py-1 text-left text-xs font-semibold text-slate-700 border-b select-none relative group",
                      !isResizing && "hover:bg-slate-200 cursor-grab active:cursor-grabbing", isResizing && "cursor-col-resize",
                      !isResizing && dragOverColumnId === header.column.id && "bg-blue-100 border-l-2 border-l-blue-500", draggedColumnId === header.column.id && "opacity-50")}
                    style={{ width: header.getSize() }}>
                    <div className="flex items-center gap-1 cursor-pointer" onClick={header.column.getToggleSortingHandler()}>
                      {flexRender(header.column.columnDef.header, header.getContext())}
                      {header.column.getCanSort() && <span className="text-slate-400">{header.column.getIsSorted() === 'asc' ? <ArrowUp className="h-3 w-3" /> : header.column.getIsSorted() === 'desc' ? <ArrowDown className="h-3 w-3" /> : <ArrowUpDown className="h-3 w-3" />}</span>}
                    </div>
                    <div onMouseDown={(e) => { e.stopPropagation(); setIsResizing(true); document.body.style.cursor = 'col-resize'; document.body.style.userSelect = 'none'; header.getResizeHandler()(e); }}
                      onTouchStart={header.getResizeHandler()} onClick={(e) => e.stopPropagation()}
            ...
            <tr className="bg-slate-50 border-b-2 border-slate-200">
              {table.getHeaderGroups()[0]?.headers.map((header) => {
                const column = header.column; const canFilter = column.getCanFilter(); const filterValue = column.getFilterValue() as string ?? '';
                const visibleFilterable = table.getAllColumns().filter(c => c.getCanFilter() && c.getIsVisible()); const filterIndex = visibleFilterable.findIndex(c => c.id === column.id);
                return (
                  <th key={header.id} className="px-1 py-1" style={{ width: header.getSize() }}>
                    {canFilter ? (
                      <div className="relative">
                        <input ref={(el) => { if (filterIndex >= 0) filterInputRefs.current[filterIndex] = el; }} type="text" value={filterValue} onChange={(e) => column.setFilterValue(e.target.value)} onFocus={() => setFocusedFilterIndex(filterIndex)} placeholder="..."
                          className={cn("w-full px-1 py-0.5 text-xs border rounded text-slate-900 placeholder-slate-400", "focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500", filterValue ? "bg-yellow-50 border-yellow-400" : "bg-white border-slate-300")} />
                        {filterValue && <button onClick={() => column.setFilterValue('')} className="absolute right-1 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><X className="h-3 w-3" /></button>}
                      </div>
                    ) : <div className="h-5" />}
                  </th>
                );
          ...
          <tbody>
            ...
            {virtualRows.map((virtualRow) => {
              const row = rows[virtualRow.index]; const isSelected = row.original.id === selectedRowId;
              return (
                <tr key={row.id} data-index={virtualRow.index}
                  className={cn('border-b transition-colors cursor-pointer', isSelected ? 'bg-blue-100 hover:bg-blue-150' : virtualRow.index % 2 === 0 ? 'bg-white hover:bg-slate-50' : 'bg-slate-50/50 hover:bg-slate-100')}
                  style={{ height: rowHeight }} onClick={() => { setSelectedRowId(row.original.id); onRowClick?.(row.original); }} onDoubleClick={() => onRowDoubleClick?.(row.original)}>
        ...
        {rows.length === 0 && <div className="flex items-center justify-center py-12 text-slate-500">{activeFiltersCount > 0 ? 'Žiadne výsledky pre zadané filtre' : 'Žiadne záznamy'}</div>}
      </div>

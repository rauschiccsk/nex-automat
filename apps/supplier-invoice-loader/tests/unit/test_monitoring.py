...
def test_global_metrics_instance():
    ...
    assert monitoring.get_metrics() is not None
...
def test_metrics_persistence():
    ...
    # Get current value
    initial = monitoring.get_metrics().invoices_processed

    # Increment
    monitoring.get_metrics().increment_processed()

    # Should persist
    assert monitoring.get_metrics().invoices_processed == initial + 1
...
def test_end_to_end_monitoring():
    ...
    monitoring.get_metrics().increment_processed()
    monitoring.get_metrics().increment_processed()
    monitoring.get_metrics().increment_failed()

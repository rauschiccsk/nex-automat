...
# ICC VM - Windows Services Installation via NSSM
# Run as Administrator!
# Usage: .\install-services-icc.ps1
...
Write-Host "  ICC - Windows Services Installation" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
Write-Host ""
...
# Service 1: NEX-Invoice-Worker-ICC
Install-NSSMService `
    -ServiceName "NEX-Invoice-Worker-ICC" `
    -Python $PYTHON_64 `
    -Arguments "-m workers.main_worker" `
    -WorkDir "$PROJECT_ROOT\apps\supplier-invoice-worker"

# Service 2: NEX-Polling-Scheduler-ICC
Install-NSSMService `
    -ServiceName "NEX-Polling-Scheduler-ICC" `
    -Python $PYTHON_64 `
    -Arguments "-m scheduler.polling_scheduler" `
    -WorkDir "$PROJECT_ROOT\apps\supplier-invoice-worker"

# Service 3: NEX-Automat-Loader-ICC
Install-NSSMService `
    -ServiceName "NEX-Automat-Loader-ICC" `
    -Python $PYTHON_32 `
    -Arguments "-m uvicorn main:app --host 0.0.0.0 --port 8002" `
...
Write-Host "    - NEX-Invoice-Worker-ICC" -ForegroundColor Gray
Write-Host "    - NEX-Polling-Scheduler-ICC" -ForegroundColor Gray
Write-Host "    - NEX-Automat-Loader-ICC" -ForegroundColor Gray
Write-Host ""
Write-Host "  Log directory: $LOG_DIR" -ForegroundColor White
Write-Host ""
Write-Host "  Commands:" -ForegroundColor White
Write-Host "    Start all:  Get-Service NEX-*-ICC | Start-Service" -ForegroundColor Gray
Write-Host "    Stop all:   Get-Service NEX-*-ICC | Stop-Service" -ForegroundColor Gray
Write-Host "    Status:     Get-Service NEX-*-ICC" -ForegroundColor Gray
Write-Host ""

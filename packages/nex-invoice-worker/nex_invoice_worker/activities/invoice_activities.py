...
Used in supplier mode for PDF-based invoice upload.
"""
...
async def upload_invoice_to_api(filename: str, content: bytes) -> UploadResult:
    ...
    payload = {"file_b64": file_b64, "filename": filename}
    ...
    async with httpx.AsyncClient(timeout=HTTP_DEFAULT_TIMEOUT_SECONDS) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                return UploadResult(success=False, filename=filename, message=error_msg)

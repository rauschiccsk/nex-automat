---
name: tdd
description: Formalized RED-GREEN-REFACTOR test-driven development loop. Invoke when starting a new feature or bug fix that has testable behavior (new endpoint, service function, validation rule, edge case).
---

# TDD — Red / Green / Refactor

Project TDD protocol. Use **before** writing any new behavior-carrying code.
Skip for pure refactors, config-only edits, documentation, and cosmetic UI tweaks.
...
3. Run the test and **confirm it fails with a meaningful error**:
   ...
   # or whichever test runner the project uses (npm test --run, etc.)
   ```
...
After REFACTOR, continue with CLAUDE.md §4 self-verification (lint, full
test suite, FE type-check). TDD narrows the feedback loop for one feature;
§4 verifies the whole commit didn't break anything else.
...
This helps reviewers spot that behavior was test-locked from the start.

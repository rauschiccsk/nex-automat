...
description: Root-cause-first debugging protocol. Invoke when a test fails, a production behaviour is wrong, a migration or build errors out, or the user reports "it doesn't work". Forces evidence collection before any code change.
---
...
land in the codebase. This skill replaces it with a four-phase protocol.
...
4. Snapshot the environment: image tag, recent migration number,
   relevant settings values, docker logs timestamp.
...
1. Write out the root cause in one sentence:
   - "The chat auto-save never fired because ``chatBuffer`` was
     captured by a stale closure — see commit `<hash>`."
...
3. Consider the **blast radius** — is this bug present in sibling
   modules (e.g. a stale-closure bug in one chat component is often
   present in a parallel chat component)?
...
## Anti-patterns (CLAUDE.md §9 reference)
